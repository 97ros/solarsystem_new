...
public class Book : MonoBehaviour {
    ...
    public Vector3 GetCurrentPagePosition(FlipMode mode)
    {
        if (mode == FlipMode.RightToLeft)
        {
            // Ottieni la posizione della pagina destra durante il movimento
            return GetRightPagePosition();
        }
        else if (mode == FlipMode.LeftToRight)
        {
            // Ottieni la posizione della pagina sinistra durante il movimento
            return GetLeftPagePosition();
        }

        // Ritorna Vector3.zero nel caso non ci sia una pagina in movimento
        return Vector3.zero;
    }

    // Questi metodi devono essere implementati in modo da ottenere la posizione delle pagine durante il movimento
    public Vector3 GetRightPagePosition()
    {
        return new Vector3(0, 0, 0); // Ritorna la posizione effettiva della pagina destra
    }

    public Vector3 GetLeftPagePosition()
    {
        return new Vector3(0, 0, 0); // Ritorna la posizione effettiva della pagina sinistra
    }

    internal Vector3 GetCurrentPagePosition()
    {
